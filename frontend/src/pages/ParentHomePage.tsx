...
export default function ParentHomePage() {
  ...
  useEffect(() => {
    if (!semesterId && semesters && semesters.length > 0) setSemesterId(semesters[0].id);
  }, [semesters, semesterId]);

  const { data: subjects } = useQuery({
    queryKey: ['my', 'subjects', { student: selectedChildId }],
    queryFn: () => listMySubjects({ student_id: selectedChildId }),
    enabled: !!selectedChildId,
  });

  const { data: grades } = useQuery({
    queryKey: ['my', 'grades', { student: selectedChildId, semesterId }],
    queryFn: () => listMyGrades({ student_id: selectedChildId, semester_id: semesterId || undefined }),
    enabled: !!selectedChildId && !!semesterId,
  });

  const { data: summary } = useQuery({
    queryKey: ['my', 'grades', 'summary', { student: selectedChildId, semesterId }],
    queryFn: () => getMyGradeSummary({ student_id: selectedChildId, semester_id: semesterId || undefined }),
    enabled: !!selectedChildId && !!semesterId,
  });
  ...
  const { data: prevGrades } = useQuery({
    queryKey: ['my', 'grades', { student: selectedChildId, semesterId: prevSemesterId }],
    queryFn: () => listMyGrades({ student_id: selectedChildId, semester_id: prevSemesterId }),
    enabled: !!selectedChildId && !!prevSemesterId && compare,
  });

  const { data: feedbacks } = useQuery({
    queryKey: ['my', 'feedbacks', { student: selectedChildId }],
    queryFn: () => listMyFeedbacks({ student_id: selectedChildId, limit: 5 }),
    enabled: !!selectedChildId,
  });

  const { data: attendance } = useQuery({
    queryKey: ['my', 'attendance', 'summary', { student: selectedChildId }],
    queryFn: () => getMyAttendanceSummary({ student_id: selectedChildId }),
    enabled: !!selectedChildId,
  });
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-semibold">학부모 대시보드</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div className="border rounded p-3">
          <div className="text-sm text-gray-600">자녀 선택</div>
          <select className="mt-1 border rounded p-1 w-full" value={selectedChildId} onChange={(e) => setSelectedChildId(e.target.value)}>
            {(children ?? []).map((c: StudentSummary) => (
              <option key={c.id} value={c.id}>{c.name} (번호 {c.student_number})</option>
            ))}
          </select>
        </div>
        <div className="border rounded p-3">
